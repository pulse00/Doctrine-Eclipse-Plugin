...
/**
 * Template store for Doctrine templates.
 * 
 * 
 * 
...
public class ProjectTemplateStore {
	...
	public ProjectTemplateStore(IProject project) {
		fInstanceStore = DoctrineUIPlugin.getDefault().getCodeTemplateStore();
		if (project == null) {
			fProjectStore = null;
		} else {
			final ScopedPreferenceStore projectSettings = new ScopedPreferenceStore(
					new ProjectScope(project), DoctrineUIPlugin.PLUGIN_ID);
	...
	public static boolean hasProjectSpecificTempates(IProject project) {
		...
		String pref = new ProjectScope(project).getNode(DoctrineUIPlugin.PLUGIN_ID).get(
				KEY, null);
